...
public interface SanPhamRepository extends JpaRepository<SanPham, Integer> {
    ...
    boolean existsByTenSanPhamIgnoreCaseAndXoaMemFalse(String tenSanPham);

    boolean existsByTenSanPhamIgnoreCaseAndXoaMemFalseAndIdNot(String tenSanPham, Integer id);
    ...
    @Query(value = """
        update san_pham
        set trang_thai_kinh_doanh = 0, ngay_cap_nhat = sysdatetime()
    ...
    @Query(value = """
        update san_pham
        set trang_thai_kinh_doanh = 1, ngay_cap_nhat = sysdatetime()
        where id_thuong_hieu = :thuongHieuId
          and xoa_mem = 0
          ...
          and (id_xuat_xu is null or exists (
              select 1
              from xuat_xu xx
          ...
          and (id_vi_tri_thi_dau is null or exists (
              select 1
              from vi_tri_thi_dau vt
              where vt.id = san_pham.id_vi_tri_thi_dau
                and vt.xoa_mem = 0
                and vt.trang_thai = 1
          ))
          and (id_phong_cach_choi is null or exists (
              select 1
              from phong_cach_choi pc
              where pc.id = san_pham.id_phong_cach_choi
                and pc.xoa_mem = 0
                and pc.trang_thai = 1
          ))
          and (id_co_giay is null or exists (
              select 1
              from co_giay cg
          ...
          and (id_chat_lieu is null or exists (
              select 1
              from chat_lieu cl
